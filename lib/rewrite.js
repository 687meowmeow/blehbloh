...
function rewriteTextBody(text, targetUrl, proxyHost, encoded) {
  ...
  // 0. Extract inline <script> blocks (without src=) and stash them with
  //    placeholders so the URL-rewriting passes below don't touch them.
  //    Absolute https?:// URLs inside inline scripts are NOT rewritten
  //    (they're often used for origin string comparisons; the client-side
  //    fetch/XHR patches catch them at runtime). Absolute paths /foo
  //    ARE rewritten to /p/<encoded>/foo.
  const scripts = [];
  text = text.replace(/<script(\s[^>]*)?>([\s\S]*?)<\/script>/gi, (m, attrs, content) => {
    ...
    // Skip non-JS scripts (type="application/json", etc.).
    ...
    // Rewrite only absolute paths (not absolute URLs).
    let rewritten = content.replace(/([\"'`])(\/[a-zA-Z0-9_\-\/\?\#\&\=\.\:\@\!\$\~\*\+\%\,]+)/g, (mm, quote, path) => {
