...
function rewriteCssUrls(css, targetUrl, encoded) {
  ...
  // Rewrite @import "foo.css" and @import url("foo.css") and
  // @import "foo.css" layer(...) and @import "foo.css" supports(...).
  // CSS @import can be a string, url(), or url(""). We match all forms.
  css = css.replace(/@import\s+(?:url\(\s*)?(['"]?)([^'"\s);]+)\1\s*\)?/gi,
    (full, quote, url) => {
      if (!url) return full;
      if (url.startsWith('data:') || url.startsWith('blob:') || url.startsWith('#')) return full;
      ...
      if (/^https?:\/\//i.test(url)) {
        ...
        // Relative URL — leave it (CSS resolution handles it).
        return full;
      }
      // Preserve the original form (string vs url()).
      if (/url\(\s*['"]?/i.test(full)) {
...
//
// IMPORTANT: inline <script> content is treated specially:
//   - We do NOT rewrite absolute https?:// URLs inside inline scripts
//     (that would break origin string comparisons like
//     `if (location.origin === "https://target")`).
//   - We DO rewrite absolute-path URLs (/foo → /p/<encoded>/foo).
//   - The full JS AST rewriter (rewriting `location`/`parent`/`top` reads)
//     runs separately in server.js.
function rewriteTextBody(text, targetUrl, proxyHost, encoded) {
  if (!text || typeof text !== 'string') return text;
  const prefix = `/p/${encoded}`;

  // 0. Extract inline <script> blocks and stash them with placeholders so
  //    the URL-rewriting passes below don't touch them.
  //    For JS scripts: rewrite absolute paths /foo → /p/<encoded>/foo.
  //      Absolute URLs https://host are NOT rewritten (preserve origin
  //      comparisons; client-side fetch/XHR patches catch them at runtime).
  //    For JSON scripts (type="application/json"): rewrite absolute paths
  //      AND absolute URLs (since JSON content often has URLs as data,
  //      not code — e.g. __NEXT_DATA__, manifest icons).
  //    For other script types (text/template, application/ld+json, etc.):
  //      also rewrite absolute paths AND absolute URLs (same reasoning).
  //    We skip only scripts with a src= attribute (external scripts are
  //    rewritten when the .js file is proxied).
  const scripts = [];
  text = text.replace(/<script(\s[^>]*)?>([\s\S]*?)<\/script>/gi, (m, attrs, content) => {
    ...
    const isJson = t === 'application/json' || t === 'application/ld+json';

    let rewritten;
    if (isJs) {
      // JS: rewrite absolute paths only.
      rewritten = content.replace(/([\"'`])(\/[a-zA-Z0-9_\-\/\?\#\&\=\.\:\@\!\$\~\*\+\%\,]+)/g, (mm, quote, path) => {
        if (path.startsWith('//')) return mm; // protocol-relative
        if (path.startsWith('/p/')) return mm; // already proxied
        return quote + prefix + path;
      });
    } else if (isJson || t === '' || t === 'text/template' || t.startsWith('text/')) {
      // JSON or template: rewrite absolute paths AND absolute URLs.
      rewritten = rewriteJsonBody(content, targetUrl, proxyHost, encoded);
    } else {
      // Other types: leave untouched.
      return m;
    }
    const placeholder = `\x00SCRIPT_${scripts.length}\x00`;
    scripts.push({ placeholder, attrs: attrs || '', content: rewritten });
    return placeholder;
  });
  ...
  // 4b. srcset / imagesrcset / data-srcset: multi-URL attributes.
  ...
  // 4d. Strip crossorigin attribute from <link rel="manifest"> (and other
  //     resource links). When the proxied page loads the manifest via
  //     <link crossorigin="use-credentials">, the browser expects the
  //     response to have Access-Control-Allow-Credentials: true. Since
  //     the manifest is same-origin (served by our proxy), we don't need
  //     crossorigin. Stripping avoids the browser rejecting the response.
  text = text.replace(/<link\b([^>]*\brel=["']manifest["'][^>]*)\bcrossorigin=["'][^"']*["']/gi, '<link$1');
  text = text.replace(/<link\b([^>]*\brel=["']icon["'][^>]*)\bcrossorigin=["'][^"']*["']/gi, '<link$1');
  text = text.replace(/<link\b([^>]*\brel=["']apple-touch-icon["'][^>]*)\bcrossorigin=["'][^"']*["']/gi, '<link$1');

  // 5. CSS url() inside inline <style> blocks.
  ...
  // 7. Restore inline <script> blocks from placeholders.
  for (const s of scripts) {
    text = text.replace(s.placeholder, `<script${s.attrs}>${s.content}</script>`);
  }

  return text;
}

// Rewrite a JSON body. Absolute paths (/foo) become /p/<encoded>/foo.
// Absolute URLs (https://host/path) become /p/<encoded-of-host>/path.
// We use a regex on the raw JSON text rather than JSON.parse + walk + re-emit
// because:
//   - JSON.parse + re-emit would re-serialize with different whitespace
//     (breaks some clients that hash the response).
//   - We need to preserve string escapes.
// The regex matches /... inside JSON string literals and rewrites only
// if it's an absolute path or absolute URL.
function rewriteJsonBody(text, targetUrl, proxyHost, encoded) {
  if (!text || typeof text !== 'string') return text;
  const prefix = `/p/${encoded}`;
  // Match string values. JSON strings start and end with `"`. We only
  // rewrite the CONTENT of strings whose value is a URL.
  // Pattern: "([^"\\]*(?:\\.[^"\\]*)*)" — matches a JSON string.
  return text.replace(/"((?:[^"\\]|\\.)*)"/g, (full, str) => {
    // Unescape the JSON string to get the raw value.
    let val;
    try { val = JSON.parse('"' + str + '"'); }
    catch (e) { return full; }
    if (!val || typeof val !== 'string') return full;
    // Skip non-URL values.
    if (!/^(https?:|wss?:|\/\/|\/)/.test(val)) return full;
    // Skip data:, blob:, mailto:, etc.
    if (/^(data:|blob:|mailto:|tel:|javascript:)/.test(val)) return full;
    let rewritten;
    if (/^https?:\/\//i.test(val)) {
      try {
        const u = new URL(val);
      ...
      const httpProto = proto === 'wss' ? 'https' : 'http';
      try {
        const u = new URL(httpProto + '://' + val.replace(/^wss?:\/\//, ''));
        rewritten = `${proto}://${proxyHost}/p/${encodeOrigin(u.origin)}` + (u.pathname === '/' ? '/' : u.pathname) + (u.search || '') + (u.hash || '');
      } catch { return full; }
      ...
      // Absolute path.
      if (val.startsWith('/p/')) return full; // already proxied
      rewritten = prefix + val;
    } else {
      return full;
    }
    if (rewritten === val) return full;
    // Re-escape for JSON.
...
function injectIntoHtml(text, encoded, targetOrigin) {
  ...
  // 3. Strip `integrity` (SRI) attributes from <script> and <link> tags.
  //    The JS AST rewriter mutates the script source → the SRI hash no
  //    longer matches → browser refuses to execute. Without this, every
  //    CDN-loaded library with an integrity hash silently fails.
  //    Also strip `nonce` (CSP nonces would block our rewriter's script
  //    if the page's CSP somehow survived).
  //    Apply iteratively because a single <script> tag can have both
  //    integrity AND nonce — a single regex pass only strips one.
  for (let i = 0; i < 4; i++) {
    const before = text;
    text = text.replace(
      /(<(?:script|link)\b[^>]*?)\s+(?:integrity|nonce)=["'][^"']*["']/gi,
      '$1'
    );
    if (text === before) break;
  }

  // 4. Rewrite <iframe srcdoc="..."> inline HTML content.
  //    srcdoc content can contain both single and double quotes (it's
  //    HTML, after all). Match the opening quote and find the matching
  //    closing quote by scanning for the closing `>` after the srcdoc.
  //    We use a function-based scanner that's smarter than a simple regex.
  text = text.replace(/<iframe\b[^>]*?\bsrcdoc=(["'])([\s\S]*?)\1[^>]*>/gi,
    (full, quote, content) => {
      // The srcdoc content is HTML-attribute-escaped (e.g. &quot;, &lt;).
      // Unescape, rewrite, re-escape.
      const unescaped = content
        ...
        .replace(/&gt;/g, '>')
        .replace(/&amp;/g, '&');
      // Rewrite using the same logic as inline HTML.
      const targetUrl2 = new URL(targetOrigin);
      const rewritten = rewriteTextBody(unescaped, targetUrl2, '', encoded);
      const reescaped = rewritten
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
        .replace(/'/g, '&apos;');
      return full.replace(content, reescaped);
    }
  );
